...
pub fn run(c: &cmd::Cli, s: &cmd::Script, _zc: Config)  {
    ...
    if s.stdin {
        ...
        zbus_lib::run_zbus_script(getfile::get_file_from_file(s.file.trim().to_string()), c, s)
    } else if ! s.uri.trim().is_empty() {
        zbus_lib::run_zbus_script(getfile::get_file_from_uri(s.uri.trim().to_string()), c, s)
