...
pub fn run(c: &cmd::Cli, pipeline: &cmd::Pipeline, aggregator: &cmd::PipelineAggregator, zc: Config)  {
    log::trace!("zbus_pipeline_aggregator::run() reached");

    if aggregator.pipeline_in.len() == 0 {
        log::error!("You did not specified pipeline names for a aggregator pipeline");
        return;
    }

    let mut argv: Vec<Dynamic> = Vec::new();
    for v in aggregator.args.clone() {
        argv.push(Dynamic::from(v));
    }

    cmd::zbus_pipeline_lib::pipeline_channel_bus("out".to_string(), aggregator.pipeline.clone(), c.clone(), zc.clone());

    for n in &aggregator.pipeline_in {
        log::debug!("Launching processor for pipeline {}", n);
        cmd::zbus_pipeline_lib::pipeline_bus_channel("in".to_string(), n.clone(), c.clone(), zc.clone());
    }
