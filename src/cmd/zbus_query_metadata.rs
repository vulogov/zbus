...
pub fn run(c: &cmd::Cli, _q: &cmd::Query, r: &cmd::QueryMetadata, zc: Config)  {
    log::trace!("zbus_query_metadata::run() reached");
    match zenoh::open(zc.clone()).res() {
        Ok(session) => {
            if r.convert {
                match cmd::zenoh_lib::get_key_from_metadata(c.platform_name.clone(), r.hostid.clone(), r.itemid.clone(), &session) {
                    Some(key) => {
